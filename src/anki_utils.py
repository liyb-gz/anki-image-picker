...
def save_image_to_note(note_id, image_data, field_name, mode, search_term, max_width=None, max_height=None):
    """
...
def get_addon_name():
    """Get the add-on folder name dynamically (works for both local dev and AnkiWeb installs)."""
    import os
...
class ConfigManager:
    ...
    def get_all_config(self):
        return self.mw.addonManager.getConfig(self.addon_name) or {}
    ...
    def get_global_defaults(self):
        config = self.get_all_config()
        default_provider = config.get("default_provider", "bing")
        return {
